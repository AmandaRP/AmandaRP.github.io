...
[Psalms](https://www.biblegateway.com/quicksearch/?quicksearch=psalms&qs_version=GNT) is a book in both the Christian Bible and the Jewish Torah. I thought it would be interesting to look at a sentiment analysis of the book. I chose to work with the "Good New Translation" of the bible as it does not have copyright issues like some of the other translations (per the [American Bible Society](https://scripture.api.bible/)). We'll use the [tidytext](https://CRAN.R-project.org/package=tidytext) and [sentimentr](https://CRAN.R-project.org/package=sentimentr) R packages to do the analysis. Let's start by reading data from [biblegateway.com](biblegateway.com) (using [rvest](https://CRAN.R-project.org/package=rvest)) and doing a bit of cleaning:
...
We see here that [Chapter `r max_sentiment$ch_num`](`r str_c("https://www.biblegateway.com/passage/?search=Psalms+", min_sentiment$ch_num, "&version=GNT")
`) is the most positive and [Chapter `r min_sentiment$ch_num`](`r str_c("https://www.biblegateway.com/passage/?search=Psalms+", max_sentiment$ch_num, "&version=GNT")`) is the most negative.  In Chapter 10, the psalmist laments about the wicked and asks God to "hear the desire of the afflicted." Here is a sample:
...
The equations describing the sentiment calculation can also be found on the [sentimentr GitHub page](https://github.com/trinker/sentimentr). With a bit of work we could do similar things with the `tidytext` package, but it's nice that it's automated in `sentimentr`.
...
Now, after taking valence shifters into account, [Chapter `r max_sentiment$ch_num`](`r str_c("https://www.biblegateway.com/passage/?search=Psalms+", max_sentiment$ch_num, "&version=GNT")`) is the most positive while [Chapter `r min_sentiment$ch_num`](`r str_c("https://www.biblegateway.com/passage/?search=Psalms+", min_sentiment$ch_num, "&version=GNT")`) is still the most negative. Chapter 67 is a short chapter (only 7 verses), a song written for the director of music. Here is a sample of verses 3 and 4:
...
Chapter 67 seems to have a similar sentiment as [Chapter 150](https://www.biblegateway.com/passage/?search=Psalm+119&version=KJV), which we identified in the previous analysis. However, I think that Chapter 67 has a greater variety of positive words. 

Additionally, between the two analyses, [Chapter `r flip_ch`](`r str_c("https://www.biblegateway.com/passage/?search=Psalms+", flip_ch, "&version=GNT")`) switched from a very slight negative sentiment (`r flip_score1`) to a somewhat positive sentiment (`r flip_score2`). This chapter had the largest score change between the two analyses. 
