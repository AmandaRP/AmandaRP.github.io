...
1. Let {$X_i$} for $i=1..n$ partition the sample space. Bayes rule says that $f(X_i|Y) = \frac{f(Y|X_i)f(X_i)}{\sum_{i=1}^{n}f(Y|X_i)f(X_i)}$. The ___ law is used in denominator when marginal, $f(Y)$, is unknown. 
