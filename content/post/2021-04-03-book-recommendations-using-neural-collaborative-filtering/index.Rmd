...
I decided to build a recommender system. 

## The technical details

A recommender system utilizes user feedback history (either "explicit" such as star ratings or "implicit" such as views, clicks, purchases, etc) to recommend items to users. Recommender systems have become ubiquitous in our society. Examples include: 

* Amazon recommends products to buy
* Facebook recommends friends to connect with
* YouTube recommends videos to watch 

In this experiment, I'll be using a Neural Collaborative Filtering (NCF) model, which is a deep learning model originally proposed by He et. al. in their [2017 paper](https://arxiv.org/pdf/1708.05031.pdf?source=post_page---------------------------). The model is implemented by the authors using (Python) Keras and is available in [this GitHub repository](https://github.com/hexiangnan/neural_collaborative_filtering). For my own fun exercise, I implemented the model using [R Keras](https://keras.rstudio.com/). You can find it in my [GitHub repository](https://github.com/AmandaRP/NeuralCollaborativeFiltering). 

The model is designed for binary feedback (that is, positive and negative user preferences). I'll be using a mixture of explicit and implicit feedback, described in more details in the [Inferring Likes and Dislikes section](#bookmark) of this post. The neural network model architecture is shown below (image from the [NCF paper](https://arxiv.org/pdf/1708.05031.pdf?source=post_page---------------------------)).


```{r, echo=FALSE,out.width="90%", out.height="90%",fig.show='hold',fig.align='center'}
...
```{r, echo=FALSE,out.width="12%",fig.cap="The top five books appearing on the most shelves",fig.show='hold',fig.align='center'}

knitr::include_graphics(c(str_c(path,"thelionthewitchandthewardrobe.jpg"),str_c(path,"thepoisonwoodbible.jpg"),str_c(path,"thecroniclesofnarnia.jpg"),str_c(path,"thefivepeopleyoumeetinheaven.jpg"),str_c(path,"unbroken.jpg")))
``` 
...
## Infering Likes and Dislikes {#bookmark}
...
After performing these steps, I filtered out users who did not have at least 3 "positive" book examples (a threshold that could be tweaked). As a result we have a total of `r num_users` users and `r num_books` books.
...
```{r, echo=FALSE,out.width="11%",fig.cap="Books that we liked",fig.show='hold',fig.align='center'}
...
```{r, echo=FALSE,out.width="11%",fig.cap="Books that we did not like",fig.show='hold',fig.align='center'}
...
As you can see, the number of "duds" was much smaller than the number of books that we liked. Francine Rivers was a popular author (I've only read one of her books, but hope to catch up soon). I would have also used the following books as positive training examples, but they were not in the data set:
[The Kremlin Conspiracy](https://www.goodreads.com/book/show/35620754-the-kremlin-conspiracy?ac=1&from_search=true&qid=TOx79fls5W&rank=1) by Joel Rosenberg
<!-- , [The 49th Mistic](https://www.goodreads.com/book/show/36548233-the-49th-mystic) by Ted Dekker,  -->
and [My Heart Belongs in Blue Ridge](https://www.goodreads.com/book/show/40402597-my-heart-belongs-in-the-blue-ridge?from_search=true&from_srp=true&qid=GIbp9eeJTH&rank=1) by Pepper Basham. 
...
The distribution of predictions for all books and users in the test is shown below (separated by "good" and "bad" books). 


```{r, echo=FALSE,out.width="90%",fig.align='center'}
knitr::include_graphics(str_c(path,"plt_preds.png"))
``` 
...
* Hit rate (HR) at *k*: The percentage of users who's "good" book appeared in their top *k* recommended books. (Recall the test set is composed of 100 "bad" and 1 "good" book for each user.)
* Normalized Discounted Cummulative Gain (NDGC) at 10: TODO

The result was `r round(hr_test, 2)` HR at 10 and `r round(ndcg_test, 2)` NDGC at 10 (evaluated on the test set). 

As a sanity check I also computed the metrics for a recommender "model" based solely on popularity (i.e. everyone's number one recommendation will be "The Lion, The Witch, and The Wardrobe" unless they already have it on their shelf), then it turns out that the HR at 10 is `r round(hr_pop, 2)` and the NDCG at 10 is `r round(ndcg_pop, 2)`. These metrics are significantly worse than those from the NCF model, so it paid to add the personalization.
...
Next, I decided to look at the rank of the top recommended book for each of the authors associated with books that my group enjoyed. The result was:
...
1. There are many other types of recommendation models to try, some of which claim to beat NCF. See [this list of must-read recommender papers](https://github.com/hongleizhang/RSPapers).
