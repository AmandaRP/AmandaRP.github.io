...
The distribution of predictions for all books and users in the test is shown below (separated for "good" and "bad" books). 
...
* Normalized Discounted Cummulative Gain at 10 (NDGC at 10): TODO

The result was `r round(hr_test, 2)` HR@10 and `r round(ndcg_test, 2)` NDGC@10 (evaluated on the test set). 

As a sanity check I also computed the metrics for a recommender "model" based solely on popularity (i.e. everyone's number one recommendation will be "The Lion, The Witch, and The Wardrobe" unless they already have it on their shelf), then it turns out that the HR@10 is `r round(hr_pop, 2)` and the NDCG@10 is `r round(ndcg_pop, 2)`. These metrics are significantly worse than those from the NCF model, so it paid to add the personalization.
...
1. There are many other types of recommendation models to try, some of which claim to beat NCF. (TODO: References?)
