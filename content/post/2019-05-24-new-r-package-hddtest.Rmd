...
Now run simulation for varying values of `delta` (in the range 1 to 160) testing both the null and alternative hypotheses for 30 replications each. For the null hypothesis simulation, we sample documents from the same newsgroup. For the simulation of the alternative hypotheis, we sample documents from the two different newsgroups. The resulting plot shows once curve for each the 60 simulations (which evaluate the p-value at each value of `delta`.)
...
Notice in the plot above that for `delta` in the range of about 40 to 100, the p-value is large when the null hypothesis is true (that is, the two sets of documents come from the same distribution, shown in blue) and small when the null hypothesis is false (that is, the two sets of documents come from different distributions, shown in red). So, our previous choice of `delta`=60 seems reasonable for making the correct conclusion. 
